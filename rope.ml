...
module Make(S : STRING)(C : CONTROL) = struct
  ...
  let rec index_from r i c = match r with
    ...
    | App (t1, t2, _, _) ->
        ...
        else index_from t2 (i - length t1) c
  ...
  let rec rindex_from r i c = match r with
    ...
    | App (t1, t2, _, _) ->
        ...
        else rindex_from t2 (i - length t1) c

  let rindex r = rindex_from r (length r)
...
(** The following is not used in HeVeA and troubles the addition of searching functions *)
