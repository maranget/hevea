...
You can find some more elaborate
\footurl{\docurl/examples/}{examples} in the on-line
documentation.
...
By default, \hevea{} knows some macros,
which can be of interest to users.
The \texttt{hevea} package defines \LaTeX{} equivalents for these
macros.

First, a few macros for URL management are provided:
...
Additionally, the \verb+\hevea+ and \verb+\hacha+ commands
output \hevea{} and \hacha{} logos, while
\verb+\texdef+ and \verb+\texlet+ commands (see
section~\ref{texmacro})
are defined as \verb+\def+ and \verb+\let+.

Finally, the \texttt{hevea.sty} contains
null definitions for all the macros that are specific to
\html{} output.
For instance the \verb+\cuthere+ macro for controlling
document cutting  is defined
as discarding its two arguments.
Section~\ref{hacha} exaplains what
\hevea{} does with this two arguments, but \LaTeX{} should certainly
ignore them.
...
\end{verbatim}
Assuming that the definition above is in a \url{\docurl/examples/smile.hva}{smile.hva} file,
the command sequence for translating
\url{\docurl/examples/smile.tex}{smile.tex} \mbox{now is}:
\begin{verbatim}
 # gpic -t < smile.tex > tmp.tex
...
constructs. These macros should be seen by \hevea{} only and
they have no \LaTeX{} equivalents defined in the \texttt{hevea.sty} file.
\begin{description}
\item[{\tt\char92 imgsrc\char91}{\it attributes}{\tt\char93}{\tt\char123}{\it
url}{\tt\char125}]
Output
\verb+<IMG SRC="+\textit{url}\verb+" +\textit{attributes}\verb+>+. The
argument \textit{url} should reduce to a valid url (usually a file
name), wheras the optional
argument \textit{attributes} should reduce to the intended attributes.
\item[{\tt\char92 anchor\char123}{\it label}{\tt\char125}]
Output \verb+<A NAME="+\textit{label}\verb+">+. The argument
...
in the directory \texttt{\docurl/doc/}
...
references right. In that case, a warning is issued.
...
reflecting actual cross referencing information.
...
Addditionally, files whose names are excluded by the \verb+-e+
command line option (see section~\ref{heveaoptions}) are not searched either.


Note the following features:
\begin{itemize}
\item If \textit{filename} is excluded with the \verb+-e+ option,
then \hevea{}
...
\index{image inclusion!in Postcript}\envindex{toimage}\comindex{imageflush}
\noindent
...
\subsection{The \texttt{color} Package}\label{color}\index{color}
%BEGIN LATEX
...
benefit from the cancelation mechanism.
For instance, to render small caps as bold fonts, one should first
redefine \verb+\sc+ by \verb+\renewcommand{\sc}{\bf}+.
\begin{htmlonly}
And now, the shape example above gets rendered as follows:
\begin{htmlout}\renewcommand{\sc}{\bf}
{\itshape italic shape \slshape slanted shape
\scshape small caps shape \upshape upright shape}
\end{htmlout}
While redifining \verb+\scshape+ with
\verb+\renewcommand{\scshape}{\bfseries}+ would yield:
\begin{htmlout}\renewcommand{\scshape}{\bfseries}
{\itshape italic shape \slshape slanted shape
\scshape small caps shape \upshape upright shape}
\end{htmlout}
\end{htmlonly}
Actually, using old-style declaration in type styles redefinition is
not bullet-proof. There remains two problems:
\begin{enumerate}
\item Several redefinitions may interact in nasty ways. For instance,
the following source probably does not yield what you want:
\begin{verbatim}
\renewcommand{\sc}{\bf}
\renewcommand{\bf}{\it}
{\bfseries bold series} {\scshape small caps shape}
\end{verbatim}
As a consequence of \LaTeX{} and \hevea{} ``late binding semantics'',
...
\end{htmlout}
\end{htmlonly}
The intended effect (\verb+\scshape+ is bold and \verb+\bfseries+ is italics),
can be achieved by using internal text-style declarations
(see section~\ref{internal}):
\begin{verbatim}
\renewcommand{\sc}{\@style{B}}
\renewcommand{\bf}{\@style{I}}
{\bfseries bold series} {\scshape small caps shape}
\end{verbatim}
\begin{htmlonly}
\begin{htmlout}
...
\end{htmlout}
\end{htmlonly}
\item The other problem is that this is much too complicated (and this
cannot be cured). In particular, since cancelation is done at the \html{}
level, a declaration belonging to one component may sometimes cancel the
effect of another that belongs to another component. However, you might
have not noticed it if I had not told you.
\end{enumerate}



\label{htmlcolor}\index{color}
...
sapces or command names, in particular the command name \verb+\{+.
...
the calling interface to basic commands is changed.

Assume for instance a document whose base style is \textit{jsc} (the
...
One  solution is to create a \texttt{jsc.hva} file which should (at
least, there are other problems) contain the following lines:
\begin{verbatim}
\input{article.hva}
\let\oldtitle=\title
...
See also section~\ref{exlet} for another example of using \verb+\let+.
...
Also note that
\texttt{make clean} also removes the \texttt{doc.haux} file and
that \texttt{hevea} runs
twice, in order to be sure that cross-references are correct.
In the case where \texttt{doc.tex} is first processed by \LaTeX{}
\emph{before} running \hevea{}, it is not necessary to run
\texttt{hevea} twice (see section~\ref{cross}).
