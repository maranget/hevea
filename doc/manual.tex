...
Another choice is using the \TeX{}-style conditional macro \verb+\ifhevea+:
\begin{verbatim}
We get:
...
Finally, note that boolean registers are compatible with simple \TeX-style
conditional macros (which \hevea{} implements). Thus, the following
source will yield twice the
word ``hevea'' in \hevea{} and twice the word ``latex'' in \LaTeX{}:
\begin{verbatim}
We get:
\begin{center}
\begin{tabular}{cc}
\ifthenelse{\boolean{hevea}}{hevea}{latex}
&
\ifhevea hevea\else latex\fi
\end{tabular}
\end{center}
\end{verbatim}
We get:
...
\verb+\ifthenelse{\boolean{+\textit{name}\verb+}}{+\textit{text$_1$}\verb+}{+\textit{text$_2$}\verb+}+\\
\end{tabular}
