...
color declaration (see section~\ref{color}):
\begin{verbatim}
...
etc. commands are in fact replaced by links to GIF images, while
they are replicated as one page in the \textit{image} file.
Then, the content of the GIF images result
from the processing of the \textit{image} file by
the companion script \texttt{imagen}, which must be run explicitely.
...
\index{color}
One can also specify colors using special type style declarations (see
next section).
