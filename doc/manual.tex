...
\gdef\htmlout{\begingroup\parskip=0pt\parindent=0pt\begin{center}\begin{quote}\myrule\par}
\gdef\endhtmlout{\par\myrule\end{quote}\end{center}\endgroup}
\end{latexonly} 
\begin{htmlonly}
...
Furhtermore, \hevea{} customization is done by writting \LaTeX{} code.
...
Either \hevea\ finds the the \filename{acmconf.sty} file (e.g. if
\filename{acmconf.sty} is in the current directory)
...
Paragraphs are rendered by a blank line and ther is no paragraph
indentation.
...
\newcommand{\open}{(}
\newcommand{\close}{)}
\open text opened by ``openpar''
and closed by ``closepar''\close.
\end{verbatim}
We get:
\begin{htmlout}
\newcommand{\open}{(}
\newcommand{\close}{)}
\open text opened by ``openpar'' and closed by
``closepar''\close.
\end{htmlout}
In the output above, the space after \verb+\openpar+ does not
...
\hevea{} try to emulate \LaTeX{} behavior in all situations, but
...
potentiel problem. However, rendering may be correct.
...
\begin{htmlonly}
Whith such redefinitions, we get:
\renewcommand{\sc}{\Huge}
\renewcommand{\cal}{\em}
\begin{htmlout}
...
\filename{hevea.sty} file that \hevea{} loads before processing any
input.
These macros are written using \LaTeX{} source code.
Having a look at the \filename{hevea.sty} file (or at the base style
...
\subsection{\hevea\ uncorrectly interprets a macro}\label{blob}
...
\section{To have it all}


\section{Making both \hevea{} and \LaTeX{} happy}\label{both}
A satifactory translation from \LaTeX{} to \html{} often requires
...
processor, file loading, comments and the \filename{html.sty} style
file.
...
As an examaple, this is how some text can be typset in purple by
...
\subsection{The \protect\filename{html.sty} style file}
The \filename{html.sty} style file is intended to be loaded by \LaTeX{}.
...
Thus, provided the \filename{html} package is loaded, the ``purple
...
Additionnally, the \verb+\imageflush+ macro that controls included images
...
While processing \filename{mydoc}\texttt{.tex}, \hevea{} echoes some
...
is generally bad and font style changes are problematic.
...
\subsection{A more serious example: using \protect\texttt{gpic}}
...
Then the \verb+\the+\textit{cmd}+ macro can be redefined.
For instance, section numbering can be turned into alphabetic style by:
\begin{verbatim}
\renewcommand{\thesection}{\alph{section}}
\end{verbatim}

Note that \TeX{} style for counters is not supported at all.
...
\begin{verbatim}
# hacha 0.0
  -o filename, make hacha output go into file ``filename'' (defaults to index.html)
\end{verbatim}
...
your remarks or critics to \mailto{Luc.Maranget@inria.fr}.
