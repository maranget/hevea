...
\newcommand{\urlex}[1]{\footurl{\heveaurl/examples/#1}{\texttt{#1}}}
\newcommand{\localurl}[1]{\footurl{\heveaurl/doc/#1}{\texttt{#1}}}
...
Information on \hevea{} is available at \oneurl{\heveaurl}.
\end{abstract}
...
includes a small \@locref{@index}{index}.
\end{htmlonly}
...
\footurl{\heveaurl/examples/}{examples} in the on-line
documentation.
...
\hevea{} simply treats \verb+\usepackage+ and optional arguments to
...
the distribution is not compatible with old \LaTeX{}.
...
make \textit{text} an hyper-link target with label \textit{label} &
echo \textit{text}\\ \hline
...
Finally, the \texttt{hevea} package contains
...
\end{verbatim}
Assuming that the definition above is in a \url{\heveaurl/examples/smile.hva}{smile.hva} file,
the command sequence for translating
\url{\heveaurl/examples/smile.tex}{smile.tex} \mbox{now is}:
\begin{verbatim}
 # gpic -t < smile.tex > tmp.tex
...
low-level style. That is either one of the sixteen conventional colors
...
mechanism, redfining \verb+\scshape+ is not.
...
\scshape small caps shape \upshape upright shape}
\end{htmlout}
While redefining directly \LaTeXe{} \verb+\scshape+ with
...
%\begin{toimage}\epsfbox{#2}\end{toimage}}{%With optional argument
%\begin{toimage}\epsfbox[#1]{#2}\end{toimage}}%
%\imageflush}
%\end{verbatim}
...
\item Options \textit{options} are ignored.
\item  The whole
...
implementation for the packages \textit{isolatin1} or
\textit{fullpage}\ldots

Users needing an implementation of a package that is widely used and
available are encouraged to contact the
\url{mailto:Luc.Maranget@inria.fr}{author}.
...
or from a properly designed \texttt{Makefile} (see section~\ref{makefile}).
...
\subsubsection{High-level \html{} style colors}\label{color:high}
One can also specify text color using special type style declarations.
...
\subsection{Fancy sectionning commands}
...
backgroud, the backgrould color saturation decreases as the sectionning
...
Sectionning command background colors can be changed by
