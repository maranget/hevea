...
\hevea{} internals. Reading section~\ref{internal},
the base style files \texttt{article.hva} and
...
Unclosed \LaTeX{} groups (\verb+{+\ldots{}) is another source
...
Finally, if the Postcript image is produced from a bitmap, it is
...
using the \verb+\imagsrc+ command 
is the most adequate.
It should be prefered over using the \textit{image} file
...
because using them incorrectly may generate cryptic error messages or
crash \hevea.
However:
\begin{itemize}
...
Unsupported constructs are the
\verb+\lengthtest+ and \verb+\isodd+ test expressions, which are
undefined.
...
\item The \verb+p{+{\it wd}\verb+}+ column specification is roughly equivalent
to \verb+l+ with word wrap enabled.
Additionnaly, entries in this column get top-aligned in the vertical direction
explicitely and paragraph break reduces to one line break inside them.
...
\cite[Section~5.3]{latexbis} and in the local documention of \LaTeX{}
modern installation.
...
table entries or table themselves.
...
If the last argument has extension \texttt{.tex} or no extension,
...
with the \texttt{.tex} extension removed when present.
\item The output basename, \textit{baseout}, is  \textit{basein}
with leading directories omitted.
\end{itemize}
...
The output of \hevea{} normally goes to the file \textit{baseout}\texttt{.html}.
...
Whereas output goes into the file
...
\html output goes into the file \texttt{file.html} in the current directory.

If there is no command line argument, or if the last command line
argument has an extension different from \texttt{.tex}, then
