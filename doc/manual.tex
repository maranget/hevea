...
considered equivalent to \filename{book}.


To translate documents that use other base style file,
\hevea{} must be
instructed to load one of the
style files it knows in place of the unknown style file.
...
Both situations are avoided by invoking \hevea{} with the known
base style file  \texttt{article.sty} as an extra argument:
\begin{verbatim}
...
Observe that the fix above works because \texttt{article.sty} and
\texttt{acmconf.sty} look the same to the document (i.e., they define
the same macros).
...
\end{figure}
Outputing symbols is performed using an \html{} extension:
...
Section~\ref{browser} explains how to configure Netscape on Unix
...
Thus, some formulaes may be rendered poorly.
...
Such a crude model cannot  failthfully emulate any \TeX{} box placement.
...
by giving the \verb+-v+ options, which instructs \hevea{} to show the
...
the \texttt{html.sty} file by default.
...
Moreover, comments provides an alternative to loading the
\textit{html} package,
since \LaTeX{} does not process comments by default.
For instance, is some url is to appear only in
the \html{} output, one can write:
\begin{verbatim}
Please, have a look at my home page%
%HEVEA\ (\url{http://para.inria.fr/\home{maranget}/})
.
\end{verbatim}
In \LaTeX{} one gets:
\begin{latexout}
\begin{showlatex}
Please, have a look at my home page%
%HEVEA\ (\oneurl{http://para.inria.fr/\home{maranget}/})
.
\end{showlatex}
\end{latexout}
\noindent While \html{} output is:
\begin{htmlout}
%HEVEA Please, have a look at my home page%
%HEVEA~(\oneurl{http://para.inria.fr/\home{maranget}/}).
%BEGIN LATEX
\epsfbox{home.ps}
%END LATEX
\end{htmlout}
Additionally, there are some comment equivalents for the
\verb+latexonly+ and \verb+toimage+ environments.
\begin{center}
...
\noindent Thus, the ``have a look at my home page'' examples can also
...
\subsection{Including postscript images}
...
However, the above solution implies modyfing the original \LaTeX{}
source code.
...
Observe that the definition is a definition and not a redefinition,
...
In the \texttt{gpic} example, modyfing user source cannot be totally avoided.
...
\end{centergpic}
\end{verbatim}
\hevea will process this source correctly (including the
definition of \verb+centergpic+), provided it is given its
...
\end{verbatim}
Assuming the definition above is in a \url{../examples/smile.sty}{\texttt{smile.sty}} file,
the command sequence for translating
\url{../examples/smile.tex}{\texttt{smile.tex}} now is:
\begin{verbatim}
# gpic -t < smile.tex > tmp.tex
# hevea smile.sty tmp.tex -o smile.html
...
(Also not that cutting macros are placed into \verb+%HEVEA+ comments,
...
\filename{html} package is loaded).
...
at \url{iso.html}{iso.html} and \url{symbol.html}{symbol.html}.
...
Sectioning commands output \html{} comments targeted to \hacha, for
cutting the document at various sectional units, see
section~\ref{hacha} for details.
...
the \texttt{display} and \texttt{math} condition that are set according
...
(normally \texttt{/usr/local/lib/hevea}).
...
\subsection{\texttt{imagen} usage}
\texttt{imagen} is a simple shell script.
\hevea{} must have been previously run as:
\begin{flushleft}
\texttt{\# hevea}\ldots{} \textit{base}\texttt{.tex}\\
or\\
\texttt{\# hevea}\ldots{} \texttt{-o} \textit{base}\texttt{.html}\ldots\\
\end{flushleft}
Then, \textit{base} is \hevea{} base output name and \hevea{} outputs
a \textit{base}\texttt{.image.tex} file.

The \texttt{imagen} script should then be run as:
\begin{flushleft}
\texttt{\# imagen} \textit{base}
\end{flushleft}
The \texttt{imagen} script calls \texttt{latex},
\texttt{dvips}, \texttt{gs}, \texttt{pnmcrop} and
\texttt{ppmtogif}.


Note that, for producing one \texttt{.gif} file per
page in the \textit{base}\texttt{.image.tex} file,
\texttt{imagen} can cut \LaTeX{} output into pages at two different
stages: when producing Postscript, or while interpreting Postscript.
The choice is made by checking whether \texttt{dvips} accepts the
\texttt{-i} option or not.
...
does not contain iso-latin1 characters above 127\ldots
...
really need Objective Caml (version 2.00) to compile them.
However, a Red~Hat 4.1
...
directory. The installation procedure stores the \texttt{hevea.sty}
...
Note that the \texttt{html.sty} file is simply copied to \hevea{}
...
at:
\begin{center}
\oneurl{http://www.loria.fr/services/tex/german/outils.html}
\end{center}
