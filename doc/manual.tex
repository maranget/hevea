...
Math accents (\verb+\hat+, \verb+\tilde+, etc.) are handled,
except the \verb+\check+ and \verb+\breve+ commands.
...
\ifhevea For instance, given the formula
\verb+\tilde{v} \cdot \vec{U}+, we get
``$\tilde{v} \cdot \vec{U}$'' in text mode and
$$\tilde{v} \cdot \vec{U}$$ in display mode.\fi

If such a rendering is considered to ugly,
one can redefine the math accent commands so that they issue color changes.
For instance, one can write:
\begin{verbatim}
\renewcommand{\tilde}[1]{{\blue#1}}
\renewcommand{\vec}[1]{{\red#1}}
\end{verbatim}
\ifhevea With such redefinitions the previous example now appears as:
{\renewcommand{\tilde}[1]{{\blue#1}}%
\renewcommand{\vec}[1]{{\red#1}}%
we get
``$\tilde{v} \cdot \vec{U}$'' in text mode and
...
Of course, such a trick probably requires looking closely at the
document to check wether it is still understandable or not.
