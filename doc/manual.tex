...
the name of the environment whose opening macro finally opened the
...
Additionnaly, formal parameters \verb+#+\textit{i} are replaced by
...
\url{../examples/smile.tex}{\texttt{smile.tex}} \mbox{now is}:
\begin{verbatim}
# gpic -t < smile.tex > tmp.tex
...
Command names follow strict \LaTeX{} syntax. That is, appart from
\verb+#+, \verb+$+, %$
...
Additionnaly, the letter sequence may either be preceded by ``\verb+@+''
...
balanced.
Optional arguments are enclosed in squares brackets \verb+[+\ldots{}
\verb+]+.
