...
\hacha also makes every page it generates a clone of its input
as regards attributes to the \verb+<BODY ...>+ opening
tag and meta-information
from the \verb+<HEAD>+\ldots{}
\verb+<\HEAD>+ block.
 see section~\ref{metadef} for examples of this replication feature.
...
\hevea{} only.
\begin{description}
\item[{\tt\char92 imgsrc\char91}{\it opt}{\tt\char93}{\tt\char123}{\it
arg}{\tt\char125}]
Output \verb+<IMG SRC="+\textit{arg}\verb+" +\textit{opt}\verb+>+. The argument
\textit{arg} should reduce to a file name, wheras the optional
argument \textit{opt} should reduce to the intended attributes.
...
Conversly, if \hevea{} attempt to load \filename{style}\texttt{.hva}
...
Cross referencing information is computed in two different manners
while processing a document \filename{mydoc}\texttt{.tex}, depending
