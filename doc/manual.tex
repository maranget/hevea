...
hypertext constructs such as hyperlinks in their documents.

\hevea{} provides high-level commands for doing this (see next section).
...
\subsection{High-level commands}\index{hyperlinks}\label{hyperlink}
...
because there is no standard way to express hyperlinks in \LaTeX{}.
That way, \hevea{} (optionnaly) depends on only one third party package:
...
\item[\protect\texttt{.hidx}]
The file \filename{mydoc}\texttt{.idx} is
normally not
...
Of course, loading the \texttt{urlhref.hva} only makes sense when
