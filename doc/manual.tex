...
\gdef\htmlout{\begingroup\linewidth=.8\linewidth\parskip=0pt\parindent=0pt\begin{quote}\myrule\par}
...
``\texttt{a.html}'' that you can visualize using an {\html} brower.
If \texttt{a.tex} contains maths symbols you need to instruct your
...
overide the ones in  \texttt{macros.tex}, provided the latter are made
...
element instruct the brower to switch to a symbol font.
...
figure~\ref{xfd} ressembles the html page located
...
by any browser, \hevea{} offers a degraded mode that ouputs text
in place of symbols.
...
Appart from containing symbols, formulas specify strong typesetting
...
elements being centerd vertically in horizontal display mode and
horizontally in vertical display mode.

Display mode is first openend by a \verb+displaymath+ environment.
...
Similarily, \hevea{} cannot render in-text arrays.
...
sice by \hevea{} semantics for \verb+\newcommand+ (see
...
Additionaly, the immage is put where it belongs by an
...
\hevea{} search path consist in the current, directory ``\texttt{.}'',
followed by directories specified by the user with the \texttt{-I}
option, and by \hevea{} library (normaly \texttt{/usr/local/lib/hevea}).
...
\item[{\tt -e} {\it filename}] Prevent \hevea{} from loading any file
...
\item \hevea{} executes the \TeX{} conditionnal macro
\verb+\frenchtrue+.
\end{itemize}
...
\item[{\tt -o} {\it filename}] Make \hevea{} output go into file {\it
filename}. By default, \hevea{} output goes into
