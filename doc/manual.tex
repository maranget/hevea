...
version 3.2. It can also output plain text or info files instead of \html.
...
\hacha{}.  This includes the content of the \verb+<HEAD>+\ldots{}
\verb+<\HEAD>+ block without the \verb+<TITLE>+\ldots{}
\verb+<\TITLE>+ block the attributes of the \verb+<BODY ...>+ opening
tags (which can be controled by redefining the \verb+\@meta+ and
\verb+\@bodyarg+ commands), and the arguments to the \verb+\htmlhead+
and \verb+\htmlfoot+ commands (see section~\ref{structure}).
...
untimely \html{} tags.
Then, references, index and footnotes are supported.

\section{Others types of output}
It is possible to translate \LaTeX\ file into other formats than
 \html\ 3.2. There are two others outputs : Plain Text and info files.

\subsection{Text}
The \LaTeX\ file is processed and converted into a plain text
formatted file. It allows some pretty-printing in plain text.

To translate into text, you must invoke \hevea as follow:
\begin{verbatim}
# hevea -text [-w <width>] myfile.tex
\end{verbatim}

Additionnally, an optionnal argument \texttt{-w <number>} sets the width of the output
for text formatting. By default, The text will be 72 characters wide.

Nearly every environments have been translated, included lists and tables.
The support is nearly the same as in \html, excepted some cases shown below:

The tables are not typesetted like in \html. The borders are put correctly.
However, if the table is too wide, an error can occur if it is bigger than
the width of the page. In such cases, you can ajust the width thanks to
the \texttt{-w} argument.

For now, maths are not supported at all in text mode. You can get very weird
results with mathematical formulas. But simple things such as subscripts
and superscripts should not cause too much trouble. There are of course
no sub or superscripts in text, but there will be no more characters than
the subscript itself.

For example, $x^2$ will be rendered as \verb+x2+, but $\int_0^1f(x)dx$ will
do something like : \verb+int01f(x)dx+, and that is not really readable.

The style changes are not made in this mode, because it is hardly
impossible to render in ASCII. So there are no italics,
 bold fonts, underlinings, no changes of size or color.
The only exception is for the verbatim environment
that puts the text inside quotes, to distinguish it more easily.


\subsection{Info}
The file format info is also supported. The input file must be a \LaTeX\ file,
and not a TeXinfo file. Thus you can make manuals in postscript,
\html, and info with only one input file.

You can translate your \LaTeX\ files
into info files using the arguments :
\begin{verbatim}
# hevea -info [-w <width>] myfile.tex
\end{verbatim}

The optionnal argument \verb+-w+ has the same meaning as for text output.

The text will be organized in nodes that follow
 the pattern of \LaTeX\ sectionning
commands. Menus are created to navigate through the sections easily

The Table of Content is automatic, so the command is ignored.
Then, references, index and footnotes are supported.
The pointers can only go to an info node, so it is impossible to
point to a particular line, like in \html.

If the resulting info file is too large, it is also cut into pieces,
and the first file \texttt{.info} contains the references for all
the nodes in the others files. The files are named \filename{myfile.info-1},
\filename{myfile.info-2},\ldots
...
Then, any input occuring before
...
\end{verbatim}
Similarily, some elements can be inserted into the output file
...
Sectinal unit levels and the default value of the \verb+secnumdepth+ counter
...
environments.  More precisely optional arguements to \verb+\item+ are
...
When occuring outside math mode, characters \verb+_+ and \verb+^+ act as
...
output to check wether the document is still understandable or not.
...
\end{itemize}
Additionnaly, the \verb+tabular*+ environment is
...
more adequate when included Postcript images were bitmap images in the first
place.
...
explicitely either by giving them as command line arguments or as
...
environments for multiline displayed
...
explicitely load the  \texttt{multind.hva} file.
...
command line option, and by \texttt{hevea} library directory
(normally \texttt{/usr/local/lib/hevea}). However,
\texttt{hevea} library directory compile-time value can be overridden
...
file is modified after beeing generated by \LaTeX{} (See,
...
\item[{\tt -pedantic}]  Be strict in interpreting \html-3.2
definition. In particular, this option disable style changes inside
...
\item[{\tt -text}] Output as a plain text format. Default output
extension is \texttt{.txt}.
\item[{\tt -w} {\it width}] Set the width for text or info output.
\item[{\tt -info}] Output as an info file. Default output extension
is \texttt{.info}.
\end{description}
...
The following people contributed to \hevea{} developement:
\begin{itemize}
