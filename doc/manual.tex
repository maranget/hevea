...
\comindex{def}
\hevea{} also accepts \TeX{} style, using
\verb+\def\+{\it name}\ldots with or without delimiting characters
(see Section~\ref{delimiting}),
with the same behavior as \verb+\newcommand+.  It is important to
notice that \hevea{} semantics for \verb+\def+ departs significantly
from \TeX{} semantics. In particular, defining a command that already
exists with \verb+\def+ fails in \hevea{}, whereas it succeeds in
\TeX{}.  Users are thus invited to use \LaTeX{} \verb+\renewcommand+
for the purpose of redefining commands.
