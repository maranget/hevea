...
\htmlhead{Coucou, header\rule{1ex}{1ex}}
\htmlfoot{\@print{<HR>
}{{\red\copyright INRIA--Rocquencourt \mailto{Luc.Maranget@inria.fr}}}}
\begin{document}
...
The \texttt{hevea.hva} resides in \hevea{} library and is loaded
...
relative position of their elements.
...
However, since in some sense, all \html{} tables are displayed,
the \verb+array+ and \verb+tabular+ environments implicitly open
display mode, thus allowing a satisfactory typesetting of formulas in
arrays.
More precisely array elements whose column format specification is
\verb+l+, \verb+c+ or \verb+r+ are typeset in display mode
(see section~\ref{arraydef}).
...
Other \LaTeX{} constructs (such as mathematical symbols) that require
special processing are defined
in \hevea{} source code.
However, most of these macro definitions can be overridden by a
redefinition.
There remains a small number of commands that cannot be changed.
They are either \hevea{} internal commands that finally output {\html}
(see section~\ref{internal}),
or key \LaTeX{} commands and environments, such as \verb+\newcounter+
or \verb+array+.
...
\hacha{}.
This includes the content of the \verb+<HEAD>+\ldots{} \verb+<\HEAD>+
block (except the \verb+<TITLE>+\ldots{} \verb+<\TITLE>+ block),
and the argument to the \verb+\@bodyargs+, \verb+\@htmlargs+,
\verb+\htmlhead+ and \verb+\htmlfoot+ commands (see
...
Normally, no processing of any kind is performed on this text.

However, if \texttt{hevea} is given the \texttt{-noiso} command line
option (see section~\ref{heveausage} some
characters (such as ``é'') get translated into \html{} entities (such as
\verb+&eacute;+).
...
described.
Internal macros occur at the last expansion stage of \hevea{}, they are
interpreted by \hevea{} main lexical analyzer and cannot
be redefined.
...
\verb+\@fontcolor+. Block-level elements delimit the effect of such
declarations.

\begin{description}
\item[{\tt\char92 @print\char123}{\it text}{\tt\char125}]
Echo \textit{text} verbatim. The word ``verbatim'' means that (almost,
see section~\ref{rawhtml})
no processing of \textit{text} occurs.
\item[{\tt\char92 @print\char123}{\it BLOCK}{\tt\char125\char123}{\it
...
The \emph{preambule} starts as soon as \hevea{} starts to operate and
ends when with \verb+\begin{document}+ constructs.
Then, any input is  before
\verb+\end{document}+ is translated to \html{}.
However, the preambule is processed
and the preambule comprises the content of the files given as command line
...
\verb+\htmlhead+ and \verb+\htmlfoot+ commands in the preambule.
...
For instance, to have a copyright notice at the end of all the \html{}
pages, it suffices to invoke the \verb+\htmlfoot+ command as follows
in the document preambule:
\begin{verbatim}
...
\verb+\lengthtest+ test expressions, which is
undefined.
...
\item[\texttt{french}] This register value reflects the \verb+-french+
...
Whether an array item is typset in display mode or not depends upon
its column specification,
the \verb+l+, \verb+c+ and~\verb+r+ specifications open display modes
...
in the horizontal direction. Their vertical alignement is
unspecified.

Entries in a column whose specification is \verb+p{+{\it wd}\verb+}+
get left-aligned in the horizonal direction and
...
By default, \hevea{} implement the \texttt{array} package
(see \cite[Section~5.3]{latexbis} and section~\ref{arraypack} in this
document), which significantlty extends the
...
\subsection{\TeX{} Conditional Macros}\label{texcond}
...
manual.
However \hevea{} recognizes à la \TeX{} conditional macros.
...
\multicolumn{1}{c}{\TeX} & \multicolumn{1}{c}{\LaTeX}\\[1em]
\hline
...
\cite[Section~5.3]{latexbis} and in the local documention of modern \LaTeX{}
installations.
...
specifications.
Here is a summary of the new column specifications and of how \hevea{}
implement them:
\begin{center}
...
do not specify vertical alignement, which therefore becomes browser
dependant.
...
file, overrides the documemnt definition.
...
attempt is made to open \textit{filename}.
\item If \textit{filename} does not have the extension \texttt{.tex},
then a first attempt is made to open \textit{filename}\texttt{.tex}.
...
\texttt{file}. The main input file is searched along \texttt{hevea}
...
Note that, as \texttt{hevea} requires a base style to operate, the
\texttt{article.hva} file is implicitly loaded when there is no
