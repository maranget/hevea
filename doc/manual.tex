...
Additionnally, the optional argument \texttt{-w <number>} sets the
...
themselves) is to ajust the formatting width, using the
...
Please note that \hevea{} translatex plain \LaTeX{} to info, and not
...
i.e., in \hevea{} case, to sectionnal units.
As a consequence all cross references lead to sectionnal unit headers.
...
\texttt{hevea} search path consists in the current directory ``\texttt{.}'',
followed by user-specified directories (with the \texttt{-I}
