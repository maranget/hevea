...
\subsection{Defining macros}\label{usermacro}

{\hevea} understands macro definitions
...
\verb+\renewcommand{+{\it name}\verb+}+ performs a definition of {\it name}.
...
By contrast with \LaTeX{}, spaces are significant
in math mode. Futhermore,
spaces after commands (such as \verb+\alpha+) are echoed in math mode,
...
environments}
The \verb+list+ environment translate to the
...
\end{flushleft}
\newcounter{coucou}
\begin{list}{$\bullet$}{\usecounter{coucou}}
\item Coucou
\item Zobi
\end{list}
