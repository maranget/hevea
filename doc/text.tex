...
flushing unit, then footnote texts are flushed (if any).
In the output, the texts themselves are surrounded by special comments
that tags them as footnote texts and record \textit{secname}.
\item Otherwise, no action is performed.
\end{itemize}
\end{itemize}
...
\verb+\footnoteflush{document}+, so as to flush any pending notes.
...
starts a new buffer for footnote texts,
saves the current flushing unit and old buffer on some internal stack, and
...
as argument), restores the flushing unit and footnote text buffer saved
by the matching \verb+\cutdef+.
\end{itemize}
Later, when running accros footnote texts in its input file, \hacha{}
sometimes put notes in a separate file.
More precisely, \hacha{} maintains two sectional unit levels.
\begin{enumerate}
\item The \emph{cutting level} is the current sectional unit where
cuts occur (as given by the relevant \verb+\cutdef+).
\item The \emph{section level} is the level of the current sectional unit
(as given by the last \verb+\cuthere+).
\end{enumerate}
...
if the cutting level and the note level differ or
if the note level is below the section level.
...
at the end of the current chapter page.\footnote{Standard chapter footnote.}
\cutdef*{subsubsection}
\subsubsection{A cutted subsubsection}
A note in a subsubsection, flushed at chapters.\footnote{Sent at
the end of \texttt{cutname.html}}
\subsubsection{Another cutted subsubsection}
Another note in a subsubsection, flushed at chapters.\footnote{Sent at
the end of \texttt{cutname.html}}
\cutend*
...
\verb+\@print+ command, but exerience showed it to be much more
error prone.
...
Hence, to achieve normal \texttt{latex} processing, while still loading
the \texttt{ifpdf} package, one writes:
\begin{verbatim}
\usepackage{ifpdf}
%HEVEA\pdffalse
\end{verbatim}
