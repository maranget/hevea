...
However, one can also proceeed more directely, by appending new definitions to the \emph{document style
...
In this very document, all \texttt{verbatim} environements appear over
...
One can also change the style class  attached to a given environment
and thus control styling of environments more precisely.
...
by the \verb+\getenvclass+ command. As a consequeuce, the class
attribute of environments follow normal scoping rules.
...
end of the curent group (here, the ``\texttt{\}}'' at the end). Then, the class
...
definition, with a
conventional \html~color name (here {\ifhevea\maroon\fi marron}) or as
...
style.
\end{flashyverbatim}\end{htmlonly}

\subsection{Which Class Affects\label{whatclass} What}
We now review the various style class defined by \hevea.

\subsubsection{Simple environments}

Generally, the syling of environment~\textit{env} is performed through
the the
\verb+\getclass{+\textit{env}\verb+}+/\verb+\setclass{+\textit{env}\verb+}{+\ldots\verb+}+
mecanism, which \verb+\getclass{+\textit{env}\verb+}+ producing the
default value of \textit{env}.

Concretely, this means that most of the environments are styled with
...
quotes around RGB hexadecimal color specification.
