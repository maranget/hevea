...
\def\stylex{background:teal;padding:1ex;color:navy;border:solid navy}
\newstyle{.subsectionex}{\stylex}
\def\verbstyle{margin:2ex 1ex;backgroundcolor:\@getstylecolor{subsection}}
...
\subsection{Overview}
Starting with version~1.08, \hevea{} offers support for stylesheets
(of the CSS variant see~\cite{css}).

Style sheets provide enhanced expressivity. For instance, it is now possible
...
However, style sheet mostly provide \hevea{} users with an additional
mecanism to customize their documents. To do so, users should probably
get familiar with how \hevea{} uses style sheets in the first
place.

\hevea{} interest for style sheets is at the moment confined to
block-level elements (\verb+DIV+, \verb+TABLE+, \verb+H+\textit{n},
etc.).
The general principle is as follows: when an command or an
...
block-level elements apply. However, some package (such as, for
instance~\texttt{fancysection}, see Section~\ref{fancysection}) may
define them.

Users themselves can append new definitions to the \emph{document style
sheet}, using the command \verb+\newstyle+.
...
will appear on a light blue background, text will be dark blue, some padding
...
Notice that, to yield some effet, \verb+\newstyle+~commands \emph{must} appear
...
For instance, let us consider \texttt{verbatim} environments.
\begin{alltt}
\begin{verbatim}
Inside verbatim.
\end{verbatim}
...
So far so good! But how are (style) classes (here \emph{subsection})
defined?
This is done by using special \hevea{} commands, in the preamble
...
\subsection{Controling \texttt{imagen} from document source}
