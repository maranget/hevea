...
for typesetting citation in bold, written directlty in \html:
\begin{verbatim}
...
In most situations, the anchor used is the \textit{label} argument to
...
However, the \html{} anchors used by \verb+\label+ and~\verb+\ref+ may
differ from the arguments to these commands.
More specifically, when \verb+\label{+\textit{label}\verb+}+ occurs
inside the argument of a sectioning command (\emph{i.e.} in section title,
as recommended by section~\ref{section:section}),
then the anchor used is generated by \hevea{} and differs from \textit{label}.
...
\begin{verbatim}
<h3 class="subsection" id="sec148">B.11.2&#XA0;&#XA0;Cross-References</h3>
\end{verbatim}
That is, no \verb+<a id="cross-reference"></a>+ appears above.
Notice that \hevea{} silently changes references
such as \verb+\ref{cross-reference}+
to \verb+<a name="#sec148">+\texttt{\thesubsection}\verb+</a>+,
thereby producing an internal link to the header element~\verb+h3+.
