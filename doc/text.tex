...
\subsubsection{The Title}\index{styles for!title}
The command \verb+\maketitle+ formats the document
title whithin a \verb+TABLE+ element, with
...
\subsubsection{Enclosing Things in a Styled \texttt{DIV}}\index{styles for!miscellaneous objects}
...
limited sstyling cen be performed, by using
the environment \verb+\divstyle+.
...
(whith some padding), and to have {\ifhevea\maroon maroon} text over
...
\end{divstyle}
\end{htmlonly}
We have to problems here: first the text is black, and second
...
numerous places by \hevea{}, for instance in document titles, and look
quite safe.
\begin{htmlonly}
\begin{center}\begin{tabular}{c}
\begin{divstyle}{ruled}
...
\subsubsection{List styling with \texttt{itemize} environment}\index{styles for!lists}
\newcounter{toc}
\newenvironment{toc}
...
Please refer to section~\ref{style:hacha} for details.
...
user's document.
Additionaly, external style sheets specified with \verb+\loadcssfile+
