...
\clearpage
\cutdef{part}
\part{\label{usermanual}Tutorial}
\cutdef{section}
...
should be placed in the file ``\texttt{trouble.sty}''.
...
\subsection{\hevea{} incorrectly interprets a macro}\label{blob}\comindex{rule}
\textbf{Note~:} \hevea{} now renders the \verb+\rule+ command very similarly to \LaTeX. Hence the forthcomming section is in some sense obsolete. Nevertheless, the technique described is useful and the section is still worth reading.
...
Finally, users that experience poor rendering of the \verb+\rule+ command can also upgrade \hevea, so as to benefit from the new implementation of this
command:
\begin{center}
\newcommand{\blob}{\rule[.2ex]{1ex}{1ex}}
\blob\ Blob \blob
...
\section{\label{both}Making \hevea{} and \LaTeX{} both happy}
...
comments. As a result comment equivalent may be very practical in the document preamble, although we rather recommend using the \ahrefloc{usepackage:ref}{\texttt{\textbackslash{}usepackage}} command to define \hevea-specific commands.
...
\section{\label{hacha}Cutting your document into pieces with \hacha}
...
\part{\label{referencemanual}Reference manual}
\cutdef{section}
...
\subsection{Verbatim text: the \texttt{moreverb}, \texttt{fancyvrb} and
\texttt{verbatim} packages}
\ttindex{moreverb}{package}\ttindex{fancyvrb}{package}\ttindex{verbatim}{package}%
These packages provide new commands and environments for
processing verbatim text.
I recommend using
\ahref{\catalog/fancyvrb}{\texttt{fancyvrb}}
or \ahref{\catalog/moreverb}{\texttt{moreverb}}
rather than~\texttt{verbatim},
since \hevea~implementation are more advanced for the former packages.
Although partial, these \hevea~implementations should be sufficient for
most usages. Please notice that the \texttt{listings} package is
more adequate for typesetting computer language texts.
...
\part{\label{practical}Practical information}
\cutdef{section}
...
\item[{\tt -fix0}] Execute one iteration.
Files such as the \texttt{.haux} file are used to communicate from the previous
iteration and  to the next one.
