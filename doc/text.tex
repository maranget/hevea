...
{\newstyle{.toc#1}{list-style:none;border-left:1ex solid
#2;padding:0ex 1ex;}}
...
A shorthand for \verb+\@styleattr{+\textit{attr}\verb+}+.
...
However, one can also proceeed more directly, by appending new
...
(here \colorbox[named]{Apricot}{\texttt{Apricot}} from the \texttt{named} color model).
...
the the
\verb+\getenvclass{+\textit{env}\verb+}+/\verb+\setenvclass{+\textit{env}\verb+}{+\ldots\verb+}+
mecanism, with \verb+\getenvclass{+\textit{env}\verb+}+ producing the
...
All sectionning commands (\verb+\part+, \verb+\section+ etc.)
output~\texttt{H}\textit{n} block-level  elements, which are styled
through style classes named \texttt{part}, \texttt{section}, etc.

List making-environement introduce extra style classes for items.
More specifically, for list-making environments
\texttt{itemize} and \texttt{enumerate},
\texttt{LI} elements are styled as follows:
\begin{center}
...
\subsubsection{List styling, with itemize}
\newcounter{toc}
\newenvironment{toc}
...
Such a design may be appropriate for tables of content.
\begin{htmlonly}
\begin{toc}
\item Part~A
\begin{toc}
...
\subsection{The \label{mathpartir:package} \texttt{mathpartir} package}\index{{\tt mathpartir} package}
