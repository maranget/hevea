...
all sectionning units of your document (from \verb+\part+ to
...
\end{itemize}
Notice however, that document charset is not that important, the default
\texttt{US-ASCII} works everywhere!
\fi
...
\verb+\@charset+ directly. Please see Section~\ref{encoding} for details.
...
The commands \verb+\xleftarrow+ and \verb+\xrightarrow+ also work well
in display mode. However, the arguments are ignored when outside
display mode, and an appropriate warning is issued.
...
corresponding text-mode accents, when possible.  The \verb+\vec+
...
For instance how is one can specify \texttt{latin1} encoding, while
...
to change the sectionning command introduced by bibliographies.
...
\subsection{Support for math package \texttt{amssymb}}%
\ttindex{amssymb}{package}%
\hevea{} provides support for the \texttt{amssymb} symbols using
Unicode. However, a few symbols did not have any encoding yet
(e.g. \verb+\varsubsetneqq+, $\varsubsetneqq$), and were approximated
to their nearest kin (e.g. \verb+\subsetneqq+, $\subsetneqq$ in this
case).

There are yet others which have an Unicode definition, but not supported
by current versions of some browsers (e.g. \verb+\ltimes+,
$\ltimes$). A similar fate awaited them. However, as it is hoped that
sooner rather than later, browsers will support these entities, a
command-line option \texttt{-moreentities} can be used to activate
them. Users are urget to try out the test file
\footahref{../examples/amssymb-test.html}{amssymb-test.html}, for
details on which symbols are supported by their browsers, and whether
or not they should use the \texttt{-moreentities} option.
...
{\setenvclass{mathpar}{xmathpar}\begin{mathpar}
A-Formula \and
Longer-Formula \hva\and 
And \and The-Last-One
\end{mathpar}}
...
\ahref{http://facweb.knowlton.ohio-state.edu/pviton/support/winport.html}{window
(win32) port} of \hevea.
