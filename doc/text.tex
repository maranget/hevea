...
\end{verbatim}
where the outer DIV and TABLE are due to the fact that the display-math mode uses a DIV to center everything, and tables to set elements formulas inside. 
\begin{htmlonly}
...
\subsection{The \texttt{mathpartir} package}\index{{\tt mathpartir} package}
...
The accustomed users may skip most of the following section, but are
advised to look at \texttt{the change} that them might need to make if
using the \verb+\inferrule+ macro.
...
The \hevea{} version is simplistic: all separators
(\emph{i,e,} \verb+\and+, \verb+\par+ and \verb+\\+) produce
line-breaks. However, when prefixed by \verb+\hva+ the separators
produce horizontal space.
Hence, we can typeset the previous example on two lines only:
\begin{center}
\begin{tabular}{m{0.40\hsize}m{0.1\hsize}m{0.40\hsize}}
\begin{lstlisting}[language=C]
\begin{mathpar}
