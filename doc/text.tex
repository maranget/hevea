...
\textbf{warning}: some of them are depreciated in \html{}5, and some
of them are no longer valid, prefer CSS in \verb+span+ tags) or phrase
elements (\verb+em+, etc.) in the \html{} terminology, they are part
of the more general class of text-level elements.

\item[{\tt\char92 @styleattr\char123}{\it name}{\tt\char125\char123}{\it
attr}{\tt\char125}]
A generalisation of \verb+\@span+ above.
...
\verb+style="font-size:+\textit{int}\verb+"+ as active. Note that
\textit{int} must be a small integer in the range
\texttt{1},\texttt{2}, \ldots{} , \texttt{7}. \verb+size+ is the CSS
notation (from \verb+x-small+ to \verb+120%+ included in
a \verb+xx-large+) corresponding to \textit{int}, 3 being the 
default size (\verb+medium+).
