...
\end{verbatim}
The styles for various \LaTeX{} macros and environments can be defined by making appropriate class definitions. So, in the above case, a definition for \emph{.subsection} would amounts to specifying a style for subsections. In the absence of a definition, the default style for \emph{H1} would take effect.\\
\\
So far so good! But how are these classes (e.g. \emph{.subsection}) defined. This is done using special \hevea{} commands, in the preamble of the \LaTeX{} document, and are transferred to the \emph{HEAD} of the HTML document. It is possible to link to an external style sheet, and/or define one's own styles. As an example the command 
\begin{verbatim}
    \setHTMLstyle{.subsection}{background:blue;padding:4;color:white;border:thin solid black}
\end{verbatim}
would result in the following entry in the style definitions in the preamble of the HTML document.
\begin{verbatim}
    <STYLE TYPE="text/css">
    ...
    ...
...
\begin{enumerate}
\item We first describe how to link to external stylesheets and define styles explicitly.  
\item Then we describe the classes that affect different macros and environments. 
\item We finally describe how to define enclose a particular fragment inside a \emph{DIV} with a class. This provides a way to specify styles for certain macros and environments which cannot be stylized by defining the appripriate class (e.g. HTML tables are used extensively to set math, so a style for \emph{TABLE} would leave its mark in undesired places).
\end{enumerate}

\subsection{Importing/Making Definitions}
\subsubsection{Linking to External Style Sheets}
\index{style-sheets!\verb+\loadcssfile+}
The \hevea{} command {\tt $\setminus$loadcssfile\{...\}} allows the user to link to an external style sheet (like the LINK option for HTML). The command takes as argument, either the position of the file relative to the current directory (e.g {\tt ../abc.css}), or a url (e.g. {\tt http://www.styles.com/abc.css}), and emits the HTML text to \emph{link} to the file. As an example, the command
\begin{verbatim}
    %HEVEA \loadcssfile{../abc.css}
\end{verbatim}
produces the following HTML text in the \emph{HEAD} of the document.
\begin{verbatim}
...
As the argument is read in a \TeX-like way, it is not taken in verbose. Thus, if it contains special characters such as \# or \$, they must be specified as \verb+\#,\$+ respecively. However, since most files and urls do not use special symbols, the simple specification works more often than not.
...
There are three commands using which styles may be definied. All of them create definitions in the \emph{HEAD} of the HTML document. The user may choose the one he wants, depending on the degree of familiarity the user chooses to have with the \hevea{} internals. Needless to say, more familiarity gives more power, but also calls for a more responsible use. 

\paragraph{A common theme for sectioning.}
\index{style-sheets!\verb+\setstyles+}
The {\tt $\setminus$setstyles\{...\}} command can be used to give a common theme for the various section headings in the document. Its advantage is that it requires no knowledge of \hevea{} internals. As an example the command
\begin{verbatim}
...
\end{verbatim} 
These are the classes used by the title and the various sectioning macros. As an example the macro \verb+\part+ is implemented using \verb+H1 CLASS=part+. The details are given in the section on \emph{What Classes Affect What}.
 
Worth noting is the use of \verb+\#bbddee+ to generate \texttt{\#bbddff}. As with linking to style-sheets, the argument is not read in verbose. In fact, the same holds for all arguments for style-sheet commands, and we shall assume that user keeps them in mind hereon. 

\paragraph{Each \LaTeX{} style separately}
\index{style-sheets!\verb+\setLATEXstyle+}
One may choose to define a separate style for each/some of the \LaTeX{} sectioning elements using {\tt $\setminus$setLATEXstyle}. As an example the commands 
\begin{verbatim}
...
\end{verbatim} 
The general behaviour is to add a .(dot) in front of the first argument to generate the class. It allows the user to do a lot more than \verb+\setstyles+, such as styles for selected sectioning commands, and styles for other macros and environments (e.g \verb+itemize+) that can be stylized by \hevea.

\paragraph{Each HTML style separately}
\index{style-sheets!\verb+\setHTMLstyle+}
Users may want to define basic HTML styles (hack straight into the HTML code produced by \hevea). They can do so using the command {\tt $\setminus$setHTMLstyle}. This option involves more care, and more careful knowledge of what classes are used where, and what attributes may be inherited (transferred to inner blocks).\\
\\
...
\end{verbatim}
The main difference with \verb+\setLATEXstyle+ is that no .(dot) is added to the class name, and that definitions such as {\tt H4.x.y} can be created. \\
\\
As in HTML, the definition of {\tt H1.part} will be used over the definition of {\tt .part} (if there exists one) when \texttt{<H1 CLASS=part>} is used. Similarly, So, the definition of {\tt H3} would have no effect when \texttt{<H3 CLASS=subsection>} is used, unless {\tt .subsection} is not defined.\\
\\
...
\subsection{Which Class Affects What}
We now specify which definitions have effects on what macros. The class names closely resemble the macros and environments, and should relatively easy to keep in mind. 
...
\paragraph{Description}\index{styles for!{\tt description} environment}
Governed by classes \emph{list}, emph{dt-list} and \emph{dd-list}. Given input
\begin{verbatim}
    \begin{description}
      \item[x]{y}
    \end{description}
...
\\
However, the {\tt border} attribute can be used very effectively here, to show the nested structure of proofs. In particular each branch would have its border, and the nested sequence of boxes gives good results (see the test file \footahref{../examples/style-test.tex}{\tt style-test.tex}, and its \hevea{} rendering \footahref{../examples/style-test.html}{\tt style-test.html} for more details).
...
Due to internal use of certain environments by \hevea, they cannot be supported by defining an appropriate class in the preamble. A typical example would be the \texttt{table} environment. Defining a class \emph{.table} and using it universaly would also effect math constructs, such as \verb+\over,\frac,\xleftarrow,etc.+, besides many other things.\\
\\
...
For such constructs, limited support for style-sheets is offered by the environment \verb+\divstyle+, which takes the name of a class as an argument, and encloses the part inside using \verb+<DIV CLASS=...>+ and \verb+</DIV>+.\\
\\
...
$$
\end{htmlonly}
If another \verb+divstyle+ environment were used before the display-math, the effect would be another DIV enclosing the entire text, and other elements inside also getting affected (see the test file \footahref{../examples/style-test.tex}{style-test.tex}, and its \hevea{} rendering \footahref{../examples/style-test.html}{\tt style-test.html} for more details).
...
\end{htmlonly}
\index{\verb+\xleftarrow+}\index{\ver+\xrightarrow+}
...
$$\boxed{\frac{\pi}{2}}$$
in display-math mode. The commands \verb+\bigl,\bigr+ etc. \index{\verb+\bigl,\bigr+ etc.}are also rendered well. 
...
\begin{enumerate}
\item{$\setminus$selectlanguage : to change the language}
\item{$\setminus$iflanguage : to branch after comparing with current language}
\end{enumerate}  
...
\subsubsection{Writing {\tt hva} Files}
The languages for which {\tt .hva} files are available with the dietribution are english, french, german, austrian and czech. These may need to be modified as not all accents and hyphenation techniques are supported.\\
\\    
...
By default, the inferences have single horizontal lines (scores). This can be overridden using the $\setminus$doubleLine command. This affects only the next inference. You can make make a permanent override that applies to the rest of the current proof using $\setminus$alwaysDoubleLine. \\
\\
...
\begin{tabular}{|l|l|}
\hline
$\setminus$singleLine & $\setminus$alwaysSingleLine\\
$\setminus$noLine & $\setminus$alwaysNoLine\\
$\setminus$dashedLine & $\setminus$alwaysDashedLine\\
$\setminus$solidLine & $\setminus$alwaysSolidLine\\
$\setminus$doubleLine & $\setminus$alwaysDoubleLine\\
$\setminus$dottedLine & $\setminus$alwaysDottedLine\\
\hline
\end{tabular}
...
The $\setminus$DisplayProof command displays the proof as a text item.  This allows you to put proofs anywhere normal text might appear; for example, in a paragraph, in a table, in a tabbing environment, etc.\\
\\
...
\end{verbatim}
\textbf{Note : } There is no $\setminus$DisplayProof used here; as the closeing of the environment automatically calls the command.\\
\\
\textbf{Warning : }Any commands that set line types or set vertical or horizontal alignment that are given AFTER the $\setminus$DisplayProof command will affect the next proof, no matter how distant.
...
\hline
$\setminus$fCenter & takes this as center of & ignored, judgement centered\\
 & the judgement & as a whole\\
$\setminus$kernHyps & shifts hypothesis right by & no explicit shifts allowed\\
 & specified distance & \\
$\setminus$insertBetweenHyps & insert specified horizintal space & $\geq 3$ blank spaces between\\
 & between hypotheses & hypotheses\\
$\setminus$bottomAlignProof & vertical alignment towards & already default\\
 & the bottom & \\
$\setminus$centerAlignProof & vertical alignment towards & simply ignored\\
 & the center & \\
$\setminus$normalAlignProof & \LaTeX{} tries to align & --do--\\
 & best it can & \\
\hline
\end{tabular}
...
The package {\tt proof} is also supported by \hevea, and a brief description is given below. The original documentation for the package can be found in the style file \footahref{proof.sty}{\tt proof.sty}. Proofs are specified in left-to-right pre-traversal order, with the premises separated by \&s. For example a proof    
$$
\infer{A}{
        \infer=[\imp E]{B}{
                \frac{\Pi}{2}
...
\end{verbatim}
The labels are optional arguments and are set on the right of the inference. The standard macro is $\setminus$infer. However, there are three other variants, resulting in different methods used to separate premises and the conclusion. The table below outlines them.
...
\hline
$\setminus$infer & single line\\
$\setminus$infer= & thick line (double in \LaTeX)\\
$\setminus$infer$\star$ & no line\\
$\setminus$deduce & three vertical dots\\
\hline
\end{tabular}
\caption {Package \textsc{proof} : Separators}
\end{center}
\end{table}
\noindent\textbf{Note : } All of them, even $\setminus$infer= macro, take optional arguments.
