...
\usepackage{proof}
\usepackage{alltt}
%%Notice: both mathparir and proof define \infer and \infer*
%%We want proof \infer !!!
...
\input{version.tex}
%\colorsections{20}
%HEVEA\definecolor{part}{named}{BrickRed}
...
%HEVEA \setstyles{padding:3;background:\#bbddff;border:solid blue;color:black}
%HEVEA \setHTMLstyle{.ruled-table}{padding:5;background:\#eeddbb;border:solid black;color:white}
%HEVEA \setHTMLstyle{.part}{font-variant:small-caps;font-size:30pt}
%HEVEA \setHTMLstyle{.title}{background:\#99bbdd}
%HEVEA \setHTMLstyle{.titlemain}{font-size:26pt}
%HEVEA \setHTMLstyle{.titlerest}{font-size:16pt}
...
Recent versions of most browsers offer support for unicode entities, albeit to different extents. \hevea{} exploits this fact to translate various math symbols used in \LaTeX. As a result almost the entire set of  math symbols, including the {\tt amssymb} ones, are correctly rendered. The use of the symbol font of the {\html} browsers is no longer used, although retained for the sake for backward compatibility. 
...
Among latest additions are support for style-sheets, babel, and some proof and math packages.
...
The text inside \texttt{rawhtml} environmnents is ignored.
...
\subsection{Controling \texttt{imagen} from documment source}
\comindex{@addimagenopt}
The internal command
\verb+\@addimagenopt{+\textit{option}\verb+\verb+}+ add
the text \textit{option} to \texttt{imagen} command-line options, when
lauched atomatically by \texttt{hevea} (\emph{i.e.} when
...
\section{Support for Style \label{style-sheets}Sheets}
\index{style-sheets}
\subsection{Overview}
\hevea{} also offers support for stylesheets. The principle is that the block level elements in the HTML output corresponding to a macro or environment are opened with a particular \emph{class}. As an example the \LaTeX{} command \verb+\subsection+ is implementad in \hevea{} using the block \emph{H3}, resulting in HTML output of the form
\begin{verbatim}
    <H3 CLASS=subsection>
    ...
    </H3>
\end{verbatim}
The styles for various \LaTeX{} macros and environments can be defined
by making appropriate class definitions. So, in the above case, a
class definition for \emph{subsection} would amount to specifying a style
for subsections. In the absence of a definition, the default style for
\verb+H3+ would take effect.

So far so good! But how are these classes (e.g. \emph{subsection})
defined. This is done using special \hevea{} commands, in the preamble
of the \LaTeX{} document. These are transferred to the \verb+HEAD+ of
the \html{} document. It is possible to link to an external style sheet,
and/or define one's own styles. As an example the command
\begin{verbatim}
    \setHTMLstyle{.subsection}{background:blue;padding:4;color:white;border:thin solid black}
\end{verbatim}
would result in the following entry in the style definitions in the
preamble of the \html{} document.
\begin{verbatim}
    <STYLE TYPE="text/css">
    ...
    ...
    .subsection {background:blue;padding:4;color:white;border:thin solid black}
    ...
    ...
    </STYLE>
\end{verbatim}
We now describe the usage in more detail. 

\begin{enumerate}
\item We first describe how to link to external stylesheets and define
styles explicitly.
\item Then we describe the classes that affect different macros and
environments.
\item We finally describe how to enclose a particular fragment
inside a \verb+DIV+ block-level element, opened with a class.
This provides a way to specify
styles for certain macros and environments which cannot be stylized by
defining the appropriate class (e.g. \html{} tables are used extensively
to set math, so a style for \verb+TABLE+ would leave its mark in
undesired places).
\end{enumerate}

\subsection{Importing/Making Definitions}
\subsubsection{Linking to External Style Sheets}
\index{style-sheets!\verb+\loadcssfile+}
The \hevea{} command \verb+\loadcssfile{+\ldots\verb+}+ allows the
user to link to an external style sheet (like the LINK option for
HTML). The command takes as argument, either the position of the file
relative to the current directory (e.g {\tt ../abc.css}), or a url
(e.g. {\tt http://www.styles.com/abc.css}), and emits the HTML text to
\emph{link} to the file. As an example, the command
\begin{verbatim}
    %HEVEA \loadcssfile{../abc.css}
\end{verbatim}
produces the following \html{} text in the \verb+HEAD+ of the document.
\begin{verbatim}
    <LINK REL=STYLESHEET TYPE="text/css" HREF="../abc.css">
\end{verbatim} 
If used after the preamble, \hevea{} will ignore it after issuing a warning.

\paragraph{Argument not verbose}
As the argument is processed, it is not taken in verbose. Thus, if it
contains special characters such as \# or \$, they must be specified
as \verb+\#,\$+ respectively. A viable alternative would be to quote
the argument using the \verb+\url+ command from the \texttt{url}
package (see Section~\ref{urlpackage}).

\paragraph{Limitations}
This approach, however, has its limitations. The style-sheets linked to would probably need to be modified to be \hevea-compatible, which would involve removing many definitions which could cause unwanted effects (unless the user actually \emph{wants} the unwanted effects). As an example, a style definition for SPAN would change, among others, the style of \emph{overline}d text. This means almost all predefined style files cannot be used directly. The option has still been provided for the following reasons
\begin{enumerate}
\item{If the user can rename the classes to erase conflicts, the files will still work well.}
\item{The option is particularly useful if a set of styles is to be used for multiple documents generated using \hevea.}
\item{Future releases of \hevea{} are likely to allow more flexibility.}
\end{enumerate}

\subsubsection{Defining Your Own Styles}
There are three commands using which styles may be defined. All of
them create definitions in the \verb+HEAD+ of the \html{} document. The
user may choose the one they want, depending on the degree of
familiarity the user chooses to have with \hevea{}
internals. Needless to say, more familiarity gives more power, but
also calls for a more responsible use.

\paragraph{A common theme for sectioning.}
\index{style-sheets!\verb+\setstyles+}
The \verb+\setstyles{+\ldots\verb+}+ command can be used to give a common theme for the various section headings in the document. Its advantage is that it requires no knowledge of \hevea{} internals. As an example the command
\begin{verbatim}
    %HEVEA \setstyles{padding:4; background:\#bbddee; color:black;font: sans-serif}
\end{verbatim}
sets a common style for each of sectioning elements by defining each class to have the same style, as shown below for the \emph{article} documentstyle
\begin{verbatim}
    <STYLE TYPE="text/css">
    ..
    .title {padding:4;background:#bbddee;color:black;font:sans-serif}
    .titlemain {padding:4;background:#bbddee;color:black;font:sans-serif}
    .titlerest {padding:4;background:#bbddee;color:black;font:sans-serif}
    .part {padding:4;background:#bbddee;color:black;font:sans-serif}
    .section {padding:4;background:#bbddee;color:black;font:sans-serif}
    .subsection {padding:4;background:#bbddee;color:black;font:sans-serif}
    .subsubsection {padding:4;background:#bbddee;color:black;font:sans-serif}
    .paragraph {padding:4;background:#bbddee;color:black;font:sans-serif}
    .subparagraph {padding:4;background:#bbddee;color:black;font:sans-serif}
  ...
  </STYLE>
\end{verbatim} 
These are the classes used by the title and the various sectioning macros. As an example the macro \verb+\part+ is implemented using \verb+H1 CLASS=part+. The details are given in section~\ref{whatclass}.
 
Worth noting is the use of \verb+\#bbddee+ to generate \texttt{\#bbddff}. As with linking to style-sheets, the argument is not read in verbose. In fact, the same holds for all arguments for style-sheet commands, and we shall assume that user keeps this in mind hereon. 

\paragraph{Each \LaTeX{} style separately}
\index{style-sheets!\verb+\setLATEXstyle+}
One may choose to define a separate style for each/some of the \LaTeX{} sectioning elements using \verb+\setLATEXstyle+. As an example the commands 
\begin{verbatim}
    %HEVEA \setLATEXstyle{section}{background:green;color:white;font:sans-serif}
    %HEVEA \setLATEXstyle{titlemain}{background:\#aa9933;color:black;font:sans-serif}
\end{verbatim}
produce the class definitions
\begin{verbatim}
    .section {background:green;color:white;font:sans-serif}
    .titlemain {background:#aa9933;color:black;font:sans-serif}
\end{verbatim} 
The general behaviour of the command is to add a .(dot) in front of
its first argument to generate the class name.
Not only does this allow users to specify the styles of various
sectioning commands separately (as above), it
also allows the user to specify styles for other macros and
environments that can be stylized by \hevea{} (e.g
\verb+itemize+).


\paragraph{Each HTML style separately}
\index{style-sheets!\verb+\setHTMLstyle+}

Users may want to define basic HTML styles (hack straight into the
HTML code produced by \hevea). They can do so using the command
\verb+\setHTMLstyle+. This option involves more care, and more careful
knowledge of what classes are used where, and what attributes may be
inherited (transferred to inner blocks).

As a simple example, \hevea{} uses classes H1, H2, H3, H4 to create part, section, subsection, and subsubsection headings, respectively. A user who chooses to know this, can specify the styles of each of these as
\begin{verbatim}
    %HEVEA \setHTMLstyle{H1.part} {background:green;color:white;font:helvetica}
    %HEVEA \setHTMLstyle{.section}{background:green;color:black;font:sans-serif}
    %HEVEA \setHTMLstyle{H3}      {background:green;color:black;font:sans-serif}
    %HEVEA \setHTMLstyle{H4}      {background:green;color:black;font:sans-serif}
\end{verbatim}
and obtain the predictable
\begin{verbatim}
    H1.part  {background:green;color:white;font:helvetica}
    .section {background:green;color:white;font:sans-serif}
    H3       {background:green;color:black;font:sans-serif}
    H4.x.y   {background:green;color:black;font:sans-serif}
\end{verbatim}
The main difference with \verb+\setLATEXstyle+ is that no .(dot) is
added to the class name, and that definitions such as {\tt H4.x.y} can
be created.

As in HTML, the definition of {\tt H1.part} will take precedence over the
definition of {\tt .part} (if there exists one) when \texttt{<H1
CLASS=part>} is used. Similarly, So, the definition of {\tt H3} would
have no effect when \texttt{<H3 CLASS=subsection>} is used, unless
{\tt .subsection} is not defined.

The use of this command is recommended only for users familiar with these semantics. For a first use, the previous two commands are usually sufficient. This, howver is useful for those willing to stretch the support offered by \hevea{} to its limits.  

\subsubsection{Examples of Style Definitions}
\index{style-sheets!examples}
We now give two examples which combine the above commands.
\textbf{Example~1}
An input
\begin{verbatim}
    ...
    %HEVEA \setLATEXstyle{title}    {background:green;color:black;font:helvetica}
...
\end{verbatim}
in the preamble of the \LaTeX{} document will produce HTML code 
\begin{verbatim}
    ...
    <STYLE type="text/css">
      ...
      H1.title{background:green;color:black;font:helvetica}
      H3{background:blue;color:white}
      H5{background:cyan;color:black}
      H6{background:magenta;color:white}
    </STYLE>
\end{verbatim}


\textbf{Example~2}
An adventurous input
\begin{verbatim}
    %HEVEA \loadcssfile{x.css}
    %HEVEA \loadcssfile{y.css}
    %HEVEA \setstyles              {background:green;color:white;font:sans-serif}
    %HEVEA \setHTMLstyle {H1.part} {background:green;color:white;font:helvetica}
    %HEVEA \setLATEXstyle{section} {background:blue;color:white}
    %HEVEA \setHTMLstyle{H3}       {background:cyan;color:black}
\end{verbatim}
...
\begin{verbatim}
    ...
    <STYLE type="text/css"> 
      .title    {background:green;color:white;font:sans-serif}
      .titlemain      {background:green;color:white;font:sans-serif}
      .titlerest      {background:green;color:white;font:sans-serif}    
      .part           {background:green;color:white;font:sans-serif}
      .section        {background:green;color:white;font:sans-serif}
      .subsection     {background:green;color:white;font:sans-serif}
      .subsubsection  {background:green;color:white;font:sans-serif}
      .paragraph      {background:green;color:white;font:sans-serif}
      .subparagraph   {background:green;color:white;font:sans-serif}
      H1.part         {background:green;color:white;font:helvetica}
      .section        {background:blue;color:white}
      H3              {background:cyan;color:black}
    </STYLE>
\end{verbatim}
Here {\tt .section} is defined twice, and the last definition will be used by HTML. Similarly definitions in {\tt y.css} will override those in {\tt x.css}. Also, conflicts between {\tt H1.part} and {\tt .part} will be resolved as explained above. The order of definitions is the same as the order in the \LaTeX{} document. 


\subsection{Which Class Affects\label{whatclass} What}
We now specify which definitions have effects on what macros. The
class names closely resemble the macro and environment names, and should
be relatively easy to keep in mind.

\paragraph{Itemize}\index{styles for!{\tt itemize} environment}
Governed by classes \emph{itemize} and \emph{li-itemize}. Given input
\begin{verbatim}
    \begin{itemize}
      \item{x}
    \end{itemize}
...
\begin{verbatim}
    <UL CLASS=itemize>
      <LI CLASS=li-itemize>x
    </UL>
\end{verbatim}

\paragraph{Enumerate}\index{styles for!{\tt enumerate} environment}
Governed by classes \emph{enumerate} and \emph{li-enumerate}. Given input
\begin{verbatim}
    \begin{enumerate}
      \item{x}
    \end{enumerate}
...
\begin{verbatim}
    <OL CLASS=enumerate>
      <LI CLASS=li-enumerate>x
    </OL>
\end{verbatim}

\paragraph{Description\index{styles for!{\tt
description}}\index{styles for!{\tt list}} and list environments}
The style of the \texttt{description} environment is
governed by classes \verb+description+,
\verb+dt-description+ and \verb+dd-description+. Given input
\begin{verbatim}
    \begin{description}
      \item[x]{y}
    \end{description}
...
\begin{verbatim}
    <LIST CLASS=description>
      <DT CLASS=dt-description>x
      <DD CLASS=dd-description>y
    </LIST>
\end{verbatim}

Styles for the \texttt{list} environment, which is implemented
similarly, are managed in the same manner with
style classes \verb+list+, \verb+dt-list+ and
\verb+dd-list+ respectively.

\paragraph{Sectioning Commands}\index{styles for!sectioning commands}
The table below specifies the block level elements and respective classes.
\begin{center}
\begin{tabular}{|l|l|l|}
\hline
COMMAND&CLASS REQUIRED&HTML OUTPUT \\
\hline
$\setminus$part{...}         &part&$<$H1 CLASS=part$>$ ... $<$/H1$>$\\
$\setminus$section{...}      &section&$<$H2 CLASS=section$>$ ... $<$/H2$>$\\
$\setminus$subsection{...}   &subsection&$<$H3 CLASS=subsection$>$ ... $<$/H3$>$\\
$\setminus$subsubsection{...}&subsubsection&$<$H4 CLASS=subsubsection$>$ ... $<$/H4$>$\\
$\setminus$paragraph{...}    &paragraph&$<$H5 CLASS=paragraph$>$ ... $<$/H5$>$\\
$\setminus$subparagraph{...} &subparagraph&$<$H6 CLASS=subparagraph$>$ ... $<$/H6$>$\\
\hline
\end{tabular}
\end{center}

\paragraph{The Title}\index{styles for!title}
The command \verb+\maketitle+ uses a table with a single cell, with class \verb+title+, for display. The name of the title is displayed inside block H1, with class \verb+titlemain+, while all other information (author, date) are displayed inside block H3, with class \verb+titlerest+. As an example, if the title is to contain, the titlename, author, and the date, the HTML code will look like
\begin{verbatim}
    <TABLE ALIGN=center>
     <TR>
      <TD CLASS=title>
...
\paragraph{Proofs using `bussproofs'.}\index{styles for!{\tt bussproofs} package}
Proofs are set using tables, and the (relevant) cells of the table are opened with class {\tt bussproofs}. A particular branch of the proof tree (which is indeed a proof tree in itself), is generated by creating a table inside that cell. Because the same class is used again, the user will notice no change, and a uniform proof will be seen.
However, the {\tt border} attribute can be used very effectively here,
to show the nested structure of proofs. In particular each branch
would have its border, and the nested sequence of boxes gives good
results\ifhevea{} (see the test file \footahref{../examples/style-test.tex}{\texttt{style-test.tex}}, and its \hevea{} rendering \footahref{../examples/style-test.html}{\texttt{style-test.html}} for more details)\fi.

\paragraph{Proofs using `proof'.}\index{styles for!{\tt proof} package}
As above, proofs are set using tables, and the (relevant) cells of the table are opened with class {\tt proof}. The rest of the functionality is similar to that for {\tt bussproofs}.

\paragraph{Proofs using `mathpartir'.}\index{styles for!{\tt mathpartir} package} 
Again, proofs are set using tables, and the (relevant) cells of the table are opened with class {\tt mathpartir}. The rest of the functionality is similar to that for {\tt bussproofs}.

\subsection{Enclosing Things in a Styled DIV}\index{styles for!miscellaneous objexts}
Due to internal use of certain environments by \hevea, they cannot be
stylized by defining an appropriate class in the preamble. A typical
example would be the \texttt{tabular} environment. Defining a class
\emph{.tabular} and using it universaly would also effect math
constructs, such as \verb+\over,\frac,\xleftarrow,etc.+, besides many
other things.

\index{style-sheets!\verb+\divstyle+}
For such constructs, limited support for style-sheets is offered by
the environment \verb+\divstyle+, which takes the name of a class as
an argument, and encloses the part inside using \verb+<DIV CLASS=...>+
and \verb+</DIV>+.
...
\begin{verbatim}
    \setHTMLstyle{.ruled-table}{border:solid black; padding:5; background:\#eeddbb; color:white}
\end{verbatim}   
Enclosing a \emph{tabular} environment using \verb+divstyle+ with the class \verb+ruled-table+ as the argument, as shown below
\begin{verbatim}
    $$
...
\end{verbatim}
where the outer DIV and TABLE are due to the fact that the display-math mode uses a DIV to center everything, and tables to set elements formulae inside. 
\begin{htmlonly}
...
$$
\end{htmlonly}
If another \verb+divstyle+ environment were used before the
display-math, the effect would be another DIV enclosing the entire
text, and other elements inside also getting affected (see the test
file \footahref{../examples/style-test.tex}{style-test.tex}, and its
\hevea{} rendering
\footahref{../examples/style-test.html}{\texttt{style-test.html}} for
more details).
...
mode.  As a matter of fact, math mode is for in-text formulae. From
...
The support for unicode symbols offered by modern browsers allows to translate almost all math symbols correctly. However, some of the unicode encodings are not supported by all browsres. They are usually replaced by the best approximations. The rendering using advanced character encoding can be turned on by using the command line option \verb+-goodbreak+. 

Users are encouraged to check with the {\tt test file} to see what symbols work correctly on their browser. It is hoped that in due time, all browsers will support these entities, whereupon \verb+-goodbreak+ can be made a default.

However, the \verb+-symb+ option (which was the default earlier) allows the user to revert to rendering using iso-latin1 and symbol fonts (also disables unicode translation). The -nosymb option is now the default, and while redundant, is kept for backward compatibility. We also provide the option \verb+-noent+ which performs the same action as \verb+-symb+, but makes more sense as symbols are obsolete in \hevea.
...
\subsubsection{À la \TeX{} macros definitions}\label{texmacros}
...
The \footahref{\ctan/contrib/misc/multind.sty}{\texttt{multind}} package provides another scheme
...
\end{verbatim}
it loads {\tt babel.hva}, and sends it the saved {\tt lang-list}. The file {\tt babel.hva} then looks at each language (say{\tt  x}) in it, and loads {\tt x.hva}, which offers support for the language {\tt x}. As in \LaTeX, the last language in the list is selected as default. As an example the command
\begin{verbatim}
...
The language specific details are described in the corresponding \verb+.hva+ file, just as in the \verb+.sty+ file for \LaTeX. Users need to supply this file for their language, or modify/check the files if thay are already supplied with the distribution. The list of languages is given below.
...
Some definitions may involeve specifying unicode encodings. The map from characters to unicode can be found at \footahref{http://www.unicode.org/charts/}{http://www.unicode.org/charts/} . Most language specific unicode characters can be found in the first few files. 
...
\footahref{\ctan/contrib/supported/listings}{\texttt{listings}} package.
...
\subsection{Package \texttt{bussproofs}}\index{{\tt bussproofs} package} 
\hevea{} offers support for the {\tt bussproofs} proof package. However, the commands used for explicit alignment, and those for changing the default spacing parameters, are not supported. The next section shows a part of the original documentation. For the original version of the documentation, users are referred to the style file \footahref{bussproofs.sty}{\tt bussproofs.sty}. Users familiar with the package may skip the first few sections and move to the last one, where the commands that are not supported, or those that deviate from their \LaTeX{} behavior, are detailed.

\subsubsection{Basics}
{\tt bussproofs} allows the construction of proof trees in the style of the sequent calculus and many other proof systems. Proofs are specified in left-to-right post-traversal order. For example a proof    
\begin{prooftree}
 \AxiomC{D}
 \AxiomC{A}
 \AxiomC{B}
 ...
 \BinaryInfC{E}
\end{prooftree}
is given in the order D,A,B,C,E. The above proof tree would be displayed with the commands:
\begin{verbatim}
    \AxiomC{D}
    \AxiomC{A}
    \AxiomC{B}
    ...
    \DisplayProof
\end{verbatim}
Each line in the proof is specified according to the arity of the inference which generates it. Thus, E would be specified with a $\setminus$BinaryInf or $\setminus$BinaryInfC command. Inferences in a proof may be nullary (axioms), unary, binary, or trinary.\\
\\
\textbf{NOTE : }You must give the $\setminus$DisplayProof command to make the proof be printed.  To display a centered proof on a line by itself, put the proof inside
\begin{center}
$\setminus$begin\{center\} ... $\setminus$end\{center\}
\end{center}
There are two styles for specifying horizontal centering of lines (formulas or sequents) in a proof. 
\begin{enumerate}
\item{One format $\setminus$AxiomC\{...\} just centers the formula \{...\} in the usual way.}
\item{The other format is $\setminus$Axiom\$...$\setminus$fCenter...\$. Here, the $\setminus$fCenter specifies the center of the formula.}
\end{enumerate}
\textbf{NOTE : }$\setminus$fCenter is ignored by \hevea{} and the judgement behaves as above.\\
\\
Unary, Binary, Trinary inferences are specified with the same format as Axioms.

\subsubsection{Labels}
Often, one wishes to place a label next to an inference, usually to specify the type of inference.  These labels can be placed by using the commands $\setminus$LeftLabel\{...\} and $\setminus$RightLabel\{...\} immediately before the command which specifies the inference. For example, to generate
\begin{prooftree}
 \AxiomC{A}
 \AxiomC{B}
 \RightLabel{X}
...
\begin{verbatim}
    ...
    \DisplayProof
\end{verbatim}
\textbf{NOTE : } You can have both left and right labels on an inference.

\subsubsection{Abbreviations}
By using the optional $\setminus$EnableBpAbbreviations command, various abbreviated commands are enabled.  This allows\\
$~~~~~~~~\setminus$AX and $\setminus$AXC for $\setminus$Axiom and $\setminus$AxiomC, (resp.),\\
$~~~~~~~~\setminus$DP for $\setminus$DisplayProof,\\
$~~~~~~~~\setminus$BI and $\setminus$BIC for $\setminus$BinaryInf and $\setminus$BinaryInfC,\\
$~~~~~~~~\setminus$UI and $\setminus$UIC for $\setminus$UnaryInf  and $\setminus$UnaryInfC,\\
$~~~~~~~~\setminus$TI and $\setminus$TIC for $\setminus$TrinaryInf and $\setminus$TrinaryInfC,\\
$~~~~~~~~\setminus$LL and $\setminus$RL for $\setminus$LeftLabel and $\setminus$RightLabel.\\
\\
The enabling of these short abbreviations is OPTIONAL, since there is the possibility of conflicting with names from other macro packages.

\subsubsection{Separators}
By default, the inferences have single horizontal lines (scores). This can be overridden using the \verb+\doubleLine+ command. This affects only the next inference. You can make make a permanent override that applies to the rest of the current proof using \verb+\alwaysDoubleLine+. \\
\\
The package provides similar macros, and a complete list is given in the table below.

\begin{table}[h]
\begin{center}
\begin{tabular}{|l|l|}
\hline
\verb+\singleLine+ & \verb+\alwaysSingleLine+\\
\verb+\noLine+ & \verb+\alwaysNoLine+\\
\verb+\dashedLine+ & \verb+\alwaysDashedLine+\\
\verb+\solidLine+ & \verb+\alwaysSolidLine+\\
\verb+\doubleLine+ & \verb+\alwaysDoubleLine+\\
\verb+\dottedLine+ & \verb+\alwaysDottedLine+\\
\hline
\end{tabular}
\caption {Package \textsc{bussproofs} : Separators}
\end{center}
\end{table}

\subsubsection{Displaying Proofs}
The \verb+\DisplayProof+ command displays the proof as a text item.  This allows you to put proofs anywhere normal text might appear; for example, in a paragraph, in a table, in a tabbing environment, etc.\\
\\
For displaying proofs in a centered display use 
\begin{verbatim}
    \begin{center} ... \DisplayProof \end{center}
\end{verbatim}
Actually there is a better construction to use instead. This is to write
\begin{verbatim}
    \begin{prooftree} ... \end{prooftree}
\end{verbatim}
\textbf{Note : } There is no \verb+\DisplayProof+ used here; as the closeing of the environment automatically calls the command.\\
\\
\textbf{Warning : }Any commands that set line types or set vertical or horizontal alignment that are given AFTER the \verb+\DisplayProof+ command will affect the next proof, no matter how distant.
...
\begin{verbatim}
    ...
    \AxiomC{<sequent}         \UnaryInfC{<sequent>}
    \BinaryInfC{<sequent>}    \TrinaryInfC{<sequent>}
    \doubleLine               \alwaysDoubleLine
    \noLine                   \alwaysNoLine
    \singleLine               \alwaysSingleLine
    \solidLine                \alwaysSolidLine
    \dottedLine               \alwaysDottedLine
    \dashedLine               \alwaysDashedLine
    \LeftLabel{<label>}       \RightLabel{<label>}
    \DisplayProof
\end{verbatim}

\subsubsection{Deviation from \LaTeX{} behavior}
The, commands used for explicit alignment, and those for changing the default spacing parameters, are not supported. All proofs are horizontally aligned to the center, and vertically to the bottom. The table below details the commands.

\begin{table}[h]
\begin{center}
\begin{tabular}{|l|l|l|}
\hline
Command & Function in \LaTeX & Function in \hevea\\
\hline
\verb+\fCenter+ & takes this as center of & ignored, judgement centered\\
 & the judgement & as a whole\\
\verb+\kernHyps+ & shifts hypothesis right by & no explicit shifts allowed\\
 & specified distance & \\
\verb+\insertBetweenHyps+ & insert specified horizintal space & $\geq 3$ blank spaces between\\
 & between hypotheses & hypotheses\\
\verb+\bottomAlignProof+ & vertical alignment towards & already default\\
 & the bottom & \\
\verb+\centerAlignProof+ & vertical alignment towards & simply ignored\\
 & the center & \\
\verb+\normalAlignProof+ & \LaTeX{} tries to align & --do--\\
 & best it can & \\
\hline
\end{tabular}
\caption {Package \textsc{bussproofs} : Departure from \LaTeX{} behavior}
\end{center}
\end{table}
The other commands behave as in \LaTeX.

\subsection{Package \texttt{proof}}\index{{\tt proof} package}

The package {\tt proof} is also supported by \hevea, and a brief
description is given below. The original documentation for the package
can be found in the style file
\footahref{proof.sty}{\texttt{proof.sty}}.
 Proofs are specified in left-to-right pre-traversal order,
with the premises separated by \&s. For example a proof
...
\end{verbatim}
For examples, \ifhevea see \footahref{../examples/proof-test.tex}{{\tt proof-test.tex}}, and its \hevea{} translation \footahref{../examples/proof-test.html}{{\tt proof-test.html}}.\else try out the test file {\tt proof-test.tex} available with the distribution.\fi

The labels are optional arguments and are set on the right of the inference. The standard macro is \verb+\infer+. However, there are three other variants, resulting in different methods used to separate premises and the conclusion. The table below outlines them.

\begin{table}[h]
\begin{center}
\begin{tabular}{|l|l|}
\hline
Command & Kind of Separator\\
\hline
\verb+\infer+ & single line\\
\verb+\infer=+ & thick line (double in \LaTeX)\\
\verb+\infer*+ & no line\\
\verb+\deduce+ & three vertical dots\\
\hline
\end{tabular}
\caption {Package \textsc{proof} : Separators}
\end{center}
\end{table}
\noindent\textbf{Note : } All of them, even \verb+\infer=+ macro, take optional arguments.

\subsection{Package \texttt{mathpartir}}\index{{\tt mathpartir} package}
\hevea{} includes support for the package \texttt{mathpartir}, useful firstly for typesetting several formulae on the same line, and secondly for typesetting inference rules. For the new users, a brief \hevea compatible description (lifted from the original documentation) is given below. For more details, the user is referred to the \footahref{mathpartir.ps}{\tt original documentation} for details. 

The accustomed users may skip most of the following section, but are advised to look at \texttt{the change} that them might need to make if using the \verb+\inferrule+ macro. 

\subsubsection {The mathpar environment}\index{{\tt mathpartir} package!{\tt mathpar} environment}

The mathpar environment is a ``paragraph mode for formulae''. It  allows to typeset long list of formulas putting as many as possible on the same line: 
...
Formulae are separated by \verb"\and" (or equivalently by a blank line). 
To enforce a vertical break it sufficies to replace \verb"\and" by
\verb"\\". 

The implementation of \verb"mathpar" entirely relies on the paragraph mode for text. It starts a new paragraph, and a math formula within a paragraph, after adjusting the spacing and penalties for breaks. Then, it simply binds \verb"\and" to something like \verb"\goodbreak". \\

\noindent
\subsubsection {The inferrule macro}\index{{\tt mathpartir} package!\verb+\inferrule+}

The \verb"\inferrule" macro is designed to typeset inference rules. It should only be used in math mode (or display math mode). It takes three arguments, the first being optional, specifying the label, premises, and conclusions respectively. The premises and the conclusions are both lists of formulae, and are separated by \verb"\\". 
...
\end{verbatim}
which gives the following rendering on a large page
...
\fi
$$
and automatically give something like this on a narrower one 
like that:
\def \two {\inferrule[label]
  {one \\\\ two \\ three \\ or \\\\ more \\ premisses}
  {and \\ any \\ number \\\\ of \\ conclusions \\\\ as \\ well}
}
$$
...
\noindent
Note the dissymetry between typesetting of the premisses and of conclusions where lines closer to the center are fit first. A newline can be forced by adding an empty line \verb"\\\\", as shown below.
...
The horizontal bar separating the premises and conclusions is omitted if either the premises or the conclusions are empty. To obtain the bar, one must use \verb"{ }" instead of \verb"{}" when specifying the empty argument.\\
...
&
$$\mprset{flushleft} 
\inferrule {a \\ bbb  \hva \\ ccc \\ dddd}{e \\ ff \\\\ gg}$$
\\
\end{tabular}  

\subsubsection {Derivation trees}\index{{\tt mathpartir} package!derivation trees}

To help writing cascades of rules forming a derivation tree, inference rules
can also be aligned on their bottom line. For this, we use the star-version of \verb+\inferrule+.\\

\noindent
\textbf{NOTE : }However, there is one significant difference, that accustomed \verb+mathpartir+ users must take special note of. The premises and conclusions are now separated by \verb +\and+ instead of \verb+\\+ (used earlier). This change has no effect on the layout in \LaTeX, while allowing \hevea{} to get around a small limitation. Linebreaks are forced by \verb+\\\\+ as usual. A simple example is given below. 
$$
\begin{tabular}{m{0.45\hsize}m{0.1\hsize}m{0.45\hsize}}
\begin{lstlisting}{Ocaml}
\inferrule*
   {\inferrule* {aa \and bb}{cc}
    \and dd}
   {ee}
\end{lstlisting}
&
...
\noindent
The optional arguments here follow different semantics. They are parsed by the 
\verb"keyval" package, so as to offer a set of record-like options, as described in the table below.$$
\def \val {{\em v}}
\def \arraystretch {1.4}
\begin{tabular}{|>{\tt}c|p{0.7\hsize}|}
\hline
...
\item[{\tt -francais}] Obsolete after support for {\tt babel}. Was used to set French mode. 
\item[{\tt -entities}] Obsolete now as unicode entities are used by default to render symbols.
\item[{\tt -symb}] To use symbol fonts to render symbols. 
\item[{\tt -noent}] A logical option after the above extesion, used to turn off the use of unicode entities. 
\item[{\tt -nosymb}] Obsolete now. Used to avoid symbol font earlier, but that is so by default as entities are used. In this mode, symbols were replaced by text-only equivalents.
\item[{\tt -noiso}] Obsolete as with {\tt -nosymb}. Earlier used to not output (iso-latin1) characters whose code was above 127 (i.e. output ascii only).
...
Additionaly, if \textit{filename} is a composite filename,
\textit{dir/base}, then all files outputed by \hacha{} will
