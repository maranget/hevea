...
\ahrefurl{a.dvi} and \ahrefurl{a.html}. The \html{} output is
...
also \ahrefurl{suite.dvi}).
\item A full article, cut into pieces , with images (source is
\ahrefurl{pat.tex}, output is \ahrefurl{pat.dvi} and
\ahrefurl{pat.html}). To yield this result, \verb+gpic+ is first
...
inside other environments (files \ahrefurl{env.hva}, \ahrefurl{env.tex}
and \ahrefurl{env.html}). \texttt{gpic}, \hevea{} and \texttt{imagen}
are run as follows:
\begin{verbatim}
...
\ahrefurl{amstex.dvi} and \ahrefurl{amstex.html}.
\hevea{} is invoked as:
\begin{verbatim}
...
Caml.
\item The on-line
\url{http://loco.inria.fr/\home{georget}/clp\_fds/OnlineManual/index.html}{\texttt{clp(FD,S)}
manual}.
\end{itemize}
\end{document}
